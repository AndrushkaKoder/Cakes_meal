...
trait TextModify
{
     ...
     */
    public static function createTextArray($text, $delim = 'p', $counter = 4){
        if($text){
            if(is_numeric($delim)){
                $counter = (int)$delim;
                $delim = 'p';
     ...
     */
    public static function textCutting($str, $counter = false, $revers = false, $add_tags = true, $concat_str = ''){
    ...
    public static function textCut($str, $length = false){

        if($str){

            if(!$length || mb_strlen($str) > $length){

                $res = self::textCutting($str, $length);
    ...
    public static function wordsForCounter($counter, $arrElement = 'products'){
