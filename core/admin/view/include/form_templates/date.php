<?php if(!empty($row)):?>
    ...
    <div class="wq-block">
        ...
        <div class="wq-block__wrap">
            ...
            <input name="<?=$row?>" data-type="<?=$this->columns[$row]['Type']?>" value="<?= isset($_SESSION['res'][$row]) ? htmlspecialchars($_SESSION['res'][$row]) : (isset($this->data[$row]) ? htmlspecialchars($this->data[$row]) : '')?>" type="date" class="wq-block__input wq-block_mb-big">
        </div>
    </div>
    <!-- Конец блока "Дата" -->
<?php endif?>